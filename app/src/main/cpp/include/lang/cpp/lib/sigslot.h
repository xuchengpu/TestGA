 ...
 * THREADING MODES
 *
 *     single_threaded             - Your program is assumed to be single
 * threaded from the point of view of signal/slot usage (i.e. all objects using
 * signals and slots are created and destroyed from a single thread). Behaviour
 * if objects are destroyed concurrently is undefined (i.e. you'll get the
 * occasional segmentation fault/memory exception).
 *
 *     multi_threaded_global       - Your program is assumed to be multi
 * threaded. Objects using signals and slots can be safely created and destroyed
 * from any thread, even when connections exist. In multi_threaded_global mode,
 * this is achieved by a single global mutex (actually a critical section on
 * Windows because they are faster). This option uses less OS resources, but
 * results in more opportunities for contention, possibly resulting in more
 * context switches than are strictly necessary.
 *
 *     multi_threaded_local        - Behaviour in this mode is essentially the
 * same as multi_threaded_global, except that each signal, and each object that
 * inherits has_slots, all have their own mutex/critical section. In practice,
 * this means that mutex collisions (and hence context switches) only happen if
 * they are absolutely essential. However, on some platforms, creating a lot of
...
class _signal_base : public _signal_base_lo<mt_policy> {
 public:
  typedef typename std::list<_connection_base<mt_policy, args...>*>
    connections_list;
  ...
  void disconnect(has_slots<mt_policy>* pclass) {
    ...
    m_connected_slots.remove_if(
      [pclass, &found](_connection_base<mt_policy, args...>* x) {
        if (x->getdest() == pclass) {
          delete x;
          found = true;
          return true;
  ...
  void slot_disconnect(has_slots<mt_policy>* pslot) {
    lock_block<mt_policy> lock(&(this->m_policy));
    m_connected_slots.remove_if(
      [pslot](_connection_base<mt_policy, args...>* x) {
        if (x->getdest() == pslot) {
          delete x;
          return true;
        }
...
class _connection : public _connection_base<mt_policy, args...> {
 public:
  ...
  virtual _connection_base<mt_policy, args...>* duplicate(
    has_slots<mt_policy>* pnewdest) {
    return new _connection<dest_type, mt_policy, args...>((dest_type*)pnewdest,
                                                          m_fn);
  }
...
class signal : public internal::_signal_base<mt_policy, args...> {
 public:
  typedef
    typename internal::_signal_base<mt_policy,
                                    args...>::connections_list::const_iterator
      const_iterator;
  signal() {
  ...
  void connect(desttype* pclass, std::function<void(args...)>&& fn) {
    ...
    internal::_connection<desttype, mt_policy, args...>* conn =
      new internal::_connection<desttype, mt_policy, args...>(pclass,
                                                              std::move(fn));
  ...
  void connect(desttype* pclass, void (desttype::*memfn)(args...)) {
    this->connect(pclass, [pclass, memfn](args... a) {
      (pclass->*memfn)(a...);
    });
  }
...
template <typename mt_policy = AGORA_SIGSLOT_DEFAULT_MT_POLICY, class... args>
class repeater : public signal<mt_policy, args...>,
